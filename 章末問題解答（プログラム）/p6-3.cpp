...
public:
    ...
    Node(double val,                               // 関数値
        ...
        : v(val), d(0.0), L_Node(l_node), R_Node(r_node), L_d(l_d), R_d(r_d), v_flag(v_f) {}
    ...
    void backward(double parent_d, double parent_elemental_d) {
        ...
        if (L_Node != nullptr) {  
            ...
            if (L_Node->v_flag == true) {
                // 子ノードに対し再帰呼び出しを行う
                L_Node->backward(d, L_d);
            }

            if (R_Node != nullptr) {
                if (R_Node->v_flag == true) {
                    R_Node->backward(d, R_d);
                }
            }
...
public:
    ...
    Var(double value = 0.0, 
        ...
        : p(std::make_shared<Node>(value, node_L, node_R, ele_d_L, ele_d_R, v_f)) {}
    ...
    friend Var operator+(const Var& a, const Var& b) {
        ...
        r.p->v_flag = a.p->v_flag || b.p->v_flag; // 子ノードの変数フラグを引き継ぐ

        return r;
    }
    // 減算　（以下では，上記加算処理と同様の内容を1行で記述）
    friend Var operator-(const Var& a, const Var& b) {
        return Var(a.value() - b.value(), a.p->v_flag || b.p->v_flag, a.p, b.p, 1.0, -1.0);
    }
    // 乗算
    friend Var operator*(const Var& a, const Var& b) {
        return Var(a.value() * b.value(), a.p->v_flag || b.p->v_flag, a.p, b.p, b.value(), a.value());
    }
    // 除算 
    friend Var operator/(const Var& a, const Var& b) {
        return Var(a.value() / b.value(), a.p->v_flag || b.p->v_flag, a.p, b.p,
            1.0 / b.value(), - a.value() / (b.value()*b.value()));
    }
    // 単項マイナス演算子
    friend Var operator-(const Var& a) {
        return Var(a.value(), a.p->v_flag, a.p, nullptr, -1.0, 0.0);
    }

    // 初等関数
    friend Var sin(const Var& a) {
        return Var(std::sin(a.value()), a.p->v_flag, a.p, nullptr, std::cos(a.value()), 0.0);
    }
    friend Var exp(const Var& a) {
        return Var(std::exp(a.value()), a.p->v_flag, a.p, nullptr, std::exp(a.value()), 0.0);
    }
};
