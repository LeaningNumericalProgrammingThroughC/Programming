...
// 階乗計算（戻り値:double）-------------------------------------------------------
double factorial(unsigned long long n) {
...
// ルジャンドル多項式の値を計算--------------------------------------------------------------
double legendre(int n, double x, double a[]) {

    double P_k_m_1, P_k, P_k_p_1, s;
...
// ルジャンドル多項式の各係数の計算----------------------------------------------------------
void coefficient(double x_nodes[], double a[]) {

    double P_x_i[NP + 3], f_x[NP], weight;
    int i, j;
    ...
    // P_j(x_i)の計算を行いながら，同時にルジャンドル多項式の係数a_jの計算も行う----------
    for (i = 0; i < NP; i++) {

        // x_iに対し，各P_j(x_i)を計算する-----------------------
        P_x_i[0] = 1.0;
        P_x_i[1] = x_nodes[i];

        for (j = 1; j <= N - 1; j++) {

            P_x_i[j + 1] = ((2.0 * (double)j + 1.0) * x_nodes[i] * P_x_i[j]
                - (double)j * P_x_i[j - 1]) / ((double)j + 1.0);
        }

        // 係数a[j]を求める式におけるΣの足し込みを行う------------------------------
        weight = w(NP, x_nodes[i], P_x_i[N]);
        for (j = 0; j <= N; j++) {
            a[j] += weight * f_x[i] * P_x_i[j];
        }
    }

    // ルジャンドル多項式の各係数a_iを求めるために 1/lambda_iを掛ける----------------------
    for (i = 0; i <= N; i++) {
        a[i] *= (2.0 * i + 1.0) / 2.0;
    }
}
...
int main() {
    ...
    Complex z[NP];
    double x_nodes[NP], a[NP] = { 0.0 };
    // 多項式P(z)の係数（c[i]：x^(NP-i)の係数）
    double c[NP + 1];
    int t, i, iter, result;
    ...
    // ルジャンドル多項式の各係数を計算する-----------------------------------------------
    coefficient(x_nodes, a);

    // 結果表示---------------------------------------------------------------------------
    printf("近似多項式 ： %6.4f P_%d", a[N], N);
    for (i = N - 1; i >= 0; i--) {
        printf(" %c %6.4f P_%d", a[i] >= 0 ? '+' : '-', fabs(a[i]), i);
    }
    printf("\n\n");

    printf("   x        tan(x)        P(x)\n");
    for (double x = -1.0; x <= 1.0; x += 0.1) {
        printf("% 5.2f,   % 10.7f,   % 10.7f\n", x, f(x), legendre(N, x, a));
    }
